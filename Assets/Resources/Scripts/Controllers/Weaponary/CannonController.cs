...
public class CannonController : MonoBehaviour {
    ...
    [Header("opcional objeto que dispara")]
    ...
    private void Start(){
        bulletspeed = ammoSample.GetComponent<ApAmmoController>().bulletSpeedMs;
        createAmmo();
    ...
    private void createAmmo() {
        int ammoNeeded = Mathf.CeilToInt(ammoSample.GetComponent<ApAmmoController>().lifeTime / (60 / fireRateRPM)) + 1;
        ammo = new GameObject[ammoNeeded];
        if (launcher == null) { launcher = gameObject; }
        for (int x = 0; x < ammoNeeded; x++) {
            ammo[x] = Instantiate(ammoSample);
            if(ammo[x].GetComponent<ApAmmoController>() != null) { ammo[x].GetComponent<ApAmmoController>().launcher = launcher; }
